...
test("פנייה בלשון נקבה נתפסת — המערכת אינה יודעת את מגדר האורח", () => {
  // 🔴 שם פרטי אינו מעיד על מגדר, ולא כל שם הוא בשפה שאנחנו מזהים.
  //    פנייה מפורשת פוגעת בחצי מהאורחים בהודעה הראשונה שהם מקבלים.
  assert.ok(has("אם תרצי יציאה מאוחרת — אשמח לבדוק", "gendered-address"));
  assert.ok(has("פשוט השיבי להודעה הזו", "gendered-address"));
  assert.ok(has("נשמח מאוד אם תשתפי", "gendered-address"));

  // צורות שנכתבות זהה לשני המגדרים — **אינן** נתפסות, ובצדק.
  assert.ok(!has("היה לנו עונג לארח אותך", "gendered-address"));
  assert.ok(!has("תודה שבחרת במלון שלנו", "gendered-address"));
  assert.ok(!has("מקווים שההתמקמות הייתה נוחה", "gendered-address"));
  assert.ok(!has("הכרטיס שלך ימתין בקבלה", "gendered-address"));
  // וניסוח ניטרלי תקין עובר
  assert.ok(!has("ליציאה מאוחרת — אשמח לבדוק זמינות", "gendered-address"));
});
