...
async function sectionA() {
  ...
  // ── כמה באמת נבדק כאן? ──────────────────────────────────
  // הכוונה המקורית נכונה ונשארת: סבב שבו ה-AI רק שאל שאלת הבהרה ולא
  // חיפש אינו מוכיח דבר על בידול, ו"20 סבבים עברו" יכול להיות ריק מתוכן.
  //
  // 🔴 אבל **המדד היה שגוי**, וזה הפך את השער לתנודתי: `dualSearchRounds`
  //    מודד החלטה של ה-AI (לחפש מול לשאול שאלת הבהרה), שהיא לגיטימית
  //    לחלוטין ומשתנה מהרצה להרצה. נמדד על אותו קוד בדיוק: 7, 8, 16, 16
  //    מתוך 20. שער שנצבע אדום באקראי מלמד להתעלם ממנו — בדיוק המחלה
  //    שכבר תועדה כאן על יציבות הסוויטה (§8.11).
  //
  //    **המדד הנכון הוא `namesCompared`**, כי הוא מודד את מה שבאמת
  //    מוכיח בידול: כמה שמות מקומות *ייחודיים* הושוו בין שני המלונות
  //    בלי שאף אחד דלף. בהרצה עם 7/20 סבבים הושוו **72 שמות** — הבדיקה
  //    הייתה כל דבר חוץ מריקה.
  const enoughEvidence = namesCompared >= rounds * 2;   // ≥2 שמות לסבב, בממוצע
  check("A", "הושוו מספיק שמות מקומות ייחודיים בין המלונות (הבדיקה אינה ריקה)",
    enoughEvidence, `רק ${namesCompared} שמות הושוו ב-${rounds} סבבים — אין די ראיות להסיק בידול`);

  // כיסוי נמוך אינו כשל מוצר, אבל כן ראוי לציון: הוא אומר שההרצה הזו
  // בדקה פחות ממה שיכלה, ולא שמשהו שבור.
  if (dualSearchRounds < Math.ceil(rounds * 0.5)) {
    note(`${C.ye}⚠️ רק ${dualSearchRounds}/${rounds} סבבים הפעילו חיפוש בשני המלונות ` +
         `(ה-AI בחר לשאול שאלת הבהרה בשאר). הבידול עצמו אומת על ${namesCompared} שמות. ` +
         `להרצה יסודית יותר: npm run preflight -- 40${C.r}`);
  }
