...
// ════════════════════════════════════════════════════════
// ════════════════════════════════════════════════════════
//  שפת ברירת המחדל של המלון — כשאין שום אות שפה בהודעה
//  ----------------------------------------------------------
//  🔴 `default_lang` היה מוגדר ב-DEFAULTS מהיום הראשון ו**אף אחד לא קרא
//     אותו** — מלון שהיה מגדיר אותו לא היה מקבל כלום. זו בדיוק המחלקה
//     שכבר תועדה בפרויקט ("יש cron בתיעוד בלי cron בקוד"): הגדרה שנראית
//     קיימת ואינה מחוברת גרועה מהגדרה חסרה, כי כולם מניחים שהיא עובדת.
//
//  מתי זה בכלל נכנס לפעולה: רק כשאין *שום* אות שפה — הודעה קולית או
//  קובץ בלי כיתוב, או הודעה שכולה ספרות ("304"). הקוד נפל אז ל-"en"
//  קשיח, כך שאורח ישראלי שפתח בהודעה קולית נענה **באנגלית** במלון
//  ישראלי. עמודי ה-HTML כבר נופלים לעברית במקרה המקביל (`pageLang`);
//  עכשיו גם הצ'אט, ובאותו כלל.
//
//  "auto" = "זהה מההודעה, ואם אי אפשר — עברית". מלון בינלאומי שרוצה
//  אחרת מגדיר "en" ומקבל בדיוק את זה.
export function defaultLangFor(hotelId = HOTEL) {
  const v = String(configFor(hotelId).default_lang || "auto").toLowerCase();
  return v === "en" ? "en" : "he";
}
