...
const FORBIDDEN = [
  ...
  // 🔴 נתפס בבדיקת עמידות חיה (04.08): כששכבת המקומות נכשלה, ה-AI *ניסח
  //    מחדש לאורח את הודעת המצב הפנימית* — "The tool didn't return live
  //    results", "The live search is temporarily unavailable". זו בדיוק
  //    המחלה שכבר תועדה פעמיים בפרויקט (טקסט מרשימת ההוראות נדבק למשפט
  //    שנשלח), הפעם בצד ה-AI ובאנגלית. אורח בקמפינסקי לא יודע שקיים
  //    "כלי" או "חיפוש חי" — הוא רק שומע שהמלון לא מתפקד. הכלל הקודם
  //    כיסה עברית בלבד ("תקלה טכנית"), ולכן הדליפה האנגלית עברה בשקט.
  //    ⚠️ שמות העצם כאן הם **תשתית בלבד**. "service" הוצא במכוון: "room
  //    service is available 24/7" הוא משפט מלונאי תקין לחלוטין, וכלל
  //    שמסמן אותו היה נכבה תוך יום — וזו הדרך הבטוחה לאבד מבקר.
  [/\b(tool|live search|search tool|lookup|system|API|database|integration)\b[^.\n]{0,40}\b(is|isn['’]?t|was|wasn['’]?t|didn['’]?t|couldn['’]?t|not|no longer)\b[^.\n]{0,30}\b(available|unavailable|working|responding|respond|return|returned|reachable|down|failing|failed)\b/i,
   "מספר לאורח שמנגנון פנימי נכשל. אורח לא יודע שקיים כלי/חיפוש/מערכת — הניסוח הנכון: \"אבדוק ואחזור אליך\""],
  [/\bI\s+(couldn['’]?t|can['’]?t|was(?:n['’]?t)? able to|(?:am|was)\s+unable to|failed to)\s+(retrieve|fetch|access|load|reach|query|pull up|look .{0,6}up|connect)/i,
   "מתאר לאורח כשל טכני בגוף ראשון — הניסוח הנכון: \"אבדוק ואחזור אליך\""],
  [/(הכלי|החיפוש החי|החיפוש שלי|המערכת|מסד הנתונים|השרת)[^.\n]{0,30}(לא זמין|אינו זמין|לא החזיר|אינו מגיב|לא מגיב|נכשל|לא עובד|נפל)/,
   "מספר לאורח שמנגנון פנימי נכשל — הניסוח הנכון: \"אבדוק ואחזור אליך\""],
...
// ── פנייה בלשון נקבה — הצורות, ואיתור המילה שהפרה ──────
// מוחזרת המילה עצמה (ולא רק true) כדי שהדוח יוכל לנקוב בה. ראה ההערה
// אצל הכלל gendered-address.
...
// ⚠️ בלי `\b` — `\w` הוא ASCII ואינו עובד על עברית. הגבול נאכף בתו
//    שאינו אות עברית משני הצדדים (כך "צריך" אינו נתפס בגלל "צרי").
const GENDERED_RX = new RegExp(`(?:^|[^֐-׿])(${GENDERED_FORMS.join("|")})(?:$|[^֐-׿])`);

function genderedHit(t) {
  const m = GENDERED_RX.exec(String(t ?? ""));
  return m ? m[1] : null;
}
...
export const VOICE_RULES = [
  ...
  { id: "gendered-address", severity: "error",
    test: t => !!genderedHit(t),
    // 🔴 הדוח נהג לחתוך את ההודעה ל-110 תווים — וכשההפרה הופיעה אחריהם,
    //    הדיווח אמר "יש בעיה" בלי לומר **איזו מילה**. אבחון של הפרה כזו
    //    דרש הרצה חוזרת שלמה. עכשיו המילה עצמה בדוח.
    detail: (t) => {
      const w = genderedHit(t);
      return `פנייה בלשון נקבה ("${w}") — המערכת אינה יודעת את מגדר האורח; לנסח ניטרלית`;
    },
...
// ════════════════════════════════════════════════════════
//  ציווי ממוגדר → ניסוח ניטרלי (רשת ביטחון דטרמיניסטית)
//  ----------------------------------------------------------
//  🔴 נתפס ב-preflight חי (04.08.2026): הבוט כתב *"ספרי לי קצת יותר"*
//     לאורח ששאל "יש בר נחמד קרוב?" — משפט שאינו מסגיר מגדר בכלל.
//
//  ה-prompt כבר אוסר את זה במפורש ובפירוט ("גם ציווי הוא גוף שני ומחייב
//  מין: 'ספר לי' / 'בחר' / 'תגיד' אסורים; במקומם: 'אשמח לשמוע'…") — וזה
//  בדיוק העניין: **הנחיה לבדה אינה מספיקה**, כמו שכבר תועד כאן על `...`
//  ועל הזמנות אוכל. לכן אותה שיטה: prompt *וגם* תיקון שלא יכול להיכשל.
//
//  ⚠️ **למה רק ציווי, ולא כל נטייה.** ה-prompt מורה לבוט לזהות את מגדר
//     האורח *מתוך איך שהוא עצמו כותב* ולהתאים את עצמו ("אני עייפה" →
//     "תרצי"). זו התנהגות נכונה ורצויה, ומחיקה גורפת של צורות נקבה
//     הייתה שוברת אותה. ציווי, לעומת זאת, אסור **תמיד** — גם כשהמגדר
//     ידוע — ולכן זו הצורה היחידה שמותר לתקן בעיוורון.
//
//  ⚠️ כל תבנית דורשת "לי" אחרי הפועל, מה שהופך אותה לציווי חד-משמעי
//     המכוון לנמען. בלי זה "בחר" היה נתפס גם ב"האורח בחר" ומייצר
//     "האורח אפשר לבחור" — שכתוב שגוי גרוע מהבעיה שהוא פותר.
//     ובלי `\b`: `\w` הוא ASCII ואינו עובד על עברית.
const IMPERATIVE_FIXES = [
  [/(^|[^֐-׿])(?:תספרי|תספר|ספרי|ספר)\s+לי(?=$|[^֐-׿])/g,        "$1אשמח לשמוע"],
  [/(^|[^֐-׿])(?:תכתבי|תכתוב|כתבי|כתוב)\s+לי(?=$|[^֐-׿])/g,      "$1אפשר לכתוב לי"],
  [/(^|[^֐-׿])(?:תשלחי|תשלח|שלחי|שלח)\s+לי(?=$|[^֐-׿])/g,        "$1אפשר לשלוח לי"],
  [/(^|[^֐-׿])(?:תגידי|תגיד|הגידי|אמרי|אמור)\s+לי(?=$|[^֐-׿])/g, "$1אשמח לדעת"],
  [/(^|[^֐-׿])(?:תזכירי|תזכיר)\s+לי(?=$|[^֐-׿])/g,               "$1אשמח לתזכורת"],
  [/(^|[^֐-׿])השיבי(?=$|[^֐-׿])/g,                                "$1נא להשיב"],

  // ── ציווי נקבה ללא "לי" ────────────────────────────────
  // אלה בטוחים לתיקון גם בלי עוגן נוסף, כי צורת הציווי בנקבה (סיומת -י)
  // **אינה מתנגשת עם עבר**: "בחרי" היא ציווי בלבד, בעוד "בחר" הוא גם
  // "הוא בחר". זו בדיוק הסיבה שהצורות הזכריות המקבילות אינן כאן — שם
  // נדרש "לי" כדי להבדיל ציווי מסיפור.
  [/(^|[^֐-׿])(?:תבחרי|בחרי)(?=$|[^֐-׿])/g,   "$1אפשר לבחור"],
  [/(^|[^֐-׿])(?:תבדקי|בדקי)(?=$|[^֐-׿])/g,   "$1אפשר לבדוק"],
  [/(^|[^֐-׿])קחי(?=$|[^֐-׿])/g,              "$1אפשר לקחת"],
  [/(^|[^֐-׿])צרי\s+קשר(?=$|[^֐-׿])/g,        "$1אפשר ליצור קשר"],
  // ⚠️ "תיהני" מתוקן **רק** כשהוא עומד בפני עצמו ("תיהני!" → "בהנאה!").
  //    "תיהני מהערב" → "בהנאה מהערב" הוא עברית צולעת, ושכתוב צולע נשלח
  //    לאורח בדיוק כמו הבעיה המקורית.
  [/(^|[^֐-׿])תיהני(?=\s*[!.…]|\s*$)/g,       "$1בהנאה"],
];

// 🔴 **מה שנשאר בכוונה בחוץ, ולמה.** ל"שתפי"/"תשתפי" אין תחליף ניטרלי
//    שעובד בכל הקשר: "שתפי אותי" → "אשמח לשמוע אותי" הוא עברית שבורה
//    (נתפס בעת כתיבת הכלל). אותו דבר ל"בואי", "הגיעי" ו"תגיעי", שאינם
//    בהכרח ציווי ("כשתגיעי" הוא עתיד). כלל הברזל כאן: שכתוב חייב להיות
//    נכון **בכל** הקשר, אחרת הוא נשלח לאורח כעברית שבורה — גרוע מהבעיה
//    שהוא בא לפתור. הצורות האלה נשארות באחריות ה-prompt, ונתפסות בקול
//    ע"י `gendered-address` אם הן בכל זאת נכתבו.
//
// ⚠️ גם "תרצי"/"תוכלי" אינם כאן — במכוון, ומסיבה אחרת: ה-prompt מורה
//    לבוט להתאים את עצמו למגדר שהאורח *הסגיר בכתיבתו שלו* ("אני עייפה"
//    → "תרצי"), וזו התנהגות רצויה. מחיקה גורפת הייתה שוברת אותה.

export function neutralizeImperatives(text) {
  let out = String(text ?? "");
  for (const [re, to] of IMPERATIVE_FIXES) out = out.replace(re, to);
  return out;
}
