...
export const VOICE_RULES = [
  ...
  // ── פנייה במגדר מפורש ──────────────────────────────────
  // 🔴 המערכת **אינה יודעת** את מגדר האורח — שם פרטי אינו מעיד עליו, וגם
  //    לא כל שם הוא בשפה שאנחנו מזהים. פנייה בנטייה מפורשת ("תרצי",
  //    "השיבי") פוגעת בחצי מהאורחים בהודעה הראשונה שהם מקבלים, וזה בדיוק
  //    ההפך מרושם של מלון יוקרה.
  //
  //    הכלל תופס **נטיות נקבה בגוף שני** בלבד — הצורה היחידה שנכתבת
  //    שונה. "אותך", "שלך", "התמקמת", "שבחרת" נכתבות זהה לשני המגדרים
  //    ולכן אינן נתפסות, ובצדק.
  //
  //    ⚠️ בלי `\b` — `\w` הוא ASCII ולכן גבול-מילה אינו עובד על עברית.
  //    הגבול נאכף בתו שאינו אות עברית משני הצדדים.
  { id: "gendered-address", severity: "error",
    test: (t) => {
      const GENDERED = [
        "תרצי", "תוכלי", "תרגישי", "השיבי", "תזכירי", "תשתפי", "כתבי", "בחרי",
        "הגיעי", "בואי", "ספרי", "שלחי", "צרי", "בדקי", "אמרי", "קחי", "תבחרי",
        "תכתבי", "תשלחי", "תבדקי", "תספרי", "תגיעי", "תיהני", "תיהנו",
      ];
      const rx = new RegExp(`(^|[^֐-׿])(${GENDERED.join("|")})($|[^֐-׿])`);
      return rx.test(t);
    },
    why: "פנייה בלשון נקבה — המערכת אינה יודעת את מגדר האורח; לנסח ניטרלית" },
