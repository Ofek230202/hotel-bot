...
## 0. מצב נוכחי — קרא את זה קודם (30.07.2026)

| | |
|---|---|
| **בדיקות** | **386 עוברות, 0 נכשלות** (`npm test`) |
...
⚠️ **Postgres טרם בוצע** — `node:sqlite` סינכרוני ו-`pg` אסינכרוני, ויש
**101 קריאות DB ב-17 קבצים**. זו עבודה של 2–4 ימים עם סיכון רגרסיה. הסכימה
והתוכנית המלאה ב-**`STORE.md`**.

**המסמכים לעסק:**
- **`PMS_GUIDE.md`** — כל 13 המערכות: מה לבקש מכל מלון, איך משיגים גישה, ומה יעבוד.
  **נוצר אוטומטית** מ-`vendors.js` (`npm run pms:guide`), ולכן לא יכול להתיישן —
  יש בדיקה שנכשלת אם המדריך מפגר אחרי הקוד.
- **`OPTIMA_PMS.md`** — צלילה לאופטימה (ישראל): בעלות, המייל המדויק למלון, וצ'קליסט.
