...
## 8.17 הרשאות הופעלו בפועל — 11 תפקידים (07.08.2026)

עד כאן מערכת התפקידים הייתה **בנויה ולא פעילה**: בלי `STAFF_TOKENS`,
`DASHBOARD_PASSWORD` היחיד עבד כ-admin — ולכן בפועל הייתה **סיסמה אחת
לכל המערכת**, בדיוק מה ש-§8.13 בא לפתור. שני שינויים סגרו את זה,
ושניהם ב-env בלבד — לא בקוד:

| | |
|---|---|
| **סיסמת הדשבורד** | הוחלפה מברירת המחדל `hotel2024` לסיסמה חזקה (16 תווים · אותיות גדולות וקטנות · ספרה · תו מיוחד) |
| **`STAFF_TOKENS`** | **11 תפקידים**, טוקן אקראי של 24 תווים לכל אחד (`randomBytes(18).base64url`) |

> 🔴 **הסודות ב-`.env` בלבד** (ב-`.gitignore`), ולעולם לא ב-git. אומת:
> `git grep` על הסיסמה מחזיר אפס תוצאות, ואין קובץ סוד במעקב. בענן הם
> מוגדרים כמשתני סביבה ב-Railway.

**🆕 נוספו ארבעה תפקידים שחסרו: `spa` · `room_service` · `concierge` · `security`.**

> 🔴 **למה זה חשוב יותר ממה שזה נראה:** מחלקה שאין לה תפקיד משלה נאלצת
> לקבל טוקן של תפקיד **רחב יותר** — "כי אין ברירה". ספא שמקבל את הטוקן
> של הקבלה קורא **שיחות פרטיות של אורחים** בלי שאיש התכוון לכך. זו הדרך
> שבה הרשאות נשחקות בשטח. עכשיו יש בדיקה שנכשלת אם מחלקה ב-`DEPARTMENTS`
> נשארה בלי תפקיד משלה.

**מטריצת ההרשאות — כפי שנאכפה מול שרת חי:**

| תפקיד | שיחות | ת"ז | בקשות | דוחות | כספים | קונפיג |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| `admin` · `manager` | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| `reception` | ✅ | ⛔ | ✅ | ⛔ | ⛔ | ⛔ |
| `concierge` · `security` | ✅ | ⛔ | ✅ | ⛔ | ⛔ | ⛔ |
| `housekeeping` · `maintenance` · `spa` · `room_service` | ⛔ | ⛔ | ✅ | ⛔ | ⛔ | ⛔ |
| `accounting` | ⛔ | ⛔ | ⛔ | ✅ | ✅ | ⛔ |
| `viewer` | ⛔ | ⛔ | ✅ | ✅ | ⛔ | ⛔ |

**הבדיקה החשובה היא השלילית**, ורצה גם ביחידה וגם מול שרת אמיתי:
**אף תפקיד מלבד `admin`/`manager` אינו מגיע למסמכי זהות** · מחלקות
שירות אינן קוראות שיחות אורחים · הנהח רואה כסף ואינה קוראת שיחות · אף
תפקיד מלבד `admin` אינו מחזיק `ADMIN`. **15 בדיקות חסימה + 9 בדיקות
היתר**, כולן ירוקות מול השרת החי.

**אומת בנוסף:** הסיסמה הישנה `hotel2024` מחזירה **401** · החדשה **200** ·
טוקן שגוי **401** ("לא זוהית") ולא 403 · ו-403 נוקב ביכולת החסרה:
`{"error":"Forbidden","need":"id:view","role":"housekeeping"}`.

⚠️ **מה שנותר פתוח:** `VALIDATE_TWILIO` עדיין כבוי (התקרה פר-מלון היא
הגנת עומק, לא תחליף), ו-`ID_ENCRYPTION_KEY` אינו מוגדר — רלוונטי רק
כשמלון מפעיל `retain_image`.
