...
| | |
|---|---|
| **בדיקות** | **348 עוברות, 0 נכשלות** (`npm test`) |
...
| **PMS** | `pms/` — Mock פעיל. **Optima (ישראל) ממומש במלואו** ומחכה למפתח; Apaleo scaffold. ראה `OPTIMA_PMS.md` |
...
| `pms/index.js` | `pmsFor(hotelId)` — ספק **פר-מלון**, נפילה בטוחה ל-Mock בלי credentials |

**המסמך לעסק:** `OPTIMA_PMS.md` — מי הבעלים, איך מתחברים, ו**המייל המדויק** לשלוח למלון.
