...
| | |
|---|---|
| **בדיקות** | **424 עוברות, 0 נכשלות** (`npm test`) |
...
⚠️ **לא נבדק מול שרת אמיתי** (אין PG/Docker בסביבה) — 14 בדיקות מול לקוח
מזויף שמדבר את אותו פרוטוקול. לפני שימוש: `psql -f store/pg-schema.sql`.
⚠️ **תקרת הסקייל האמיתית אינה ה-DB** אלא שהסשנים בזיכרון התהליך. ראה `STORE.md`.
