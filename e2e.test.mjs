...
// ════════════════════════════════════════════════════════
test("צבירת תאריכים: '4 לילות' ואז '19.7' מצטרפים להגעה 19/7 + 4 לילות", async () => {
  const p = await checkinUpTo("waiting_dates");
  ...
  sent.length = 0;
  await bot.handleIncoming(p, "19.7");
  assert.match(lastBody(), /נכון\?/, "אחרי שני החלקים — מבקש אישור");
  assert.match(lastBody(), /19/, "הגעה 19/7 (מההודעה השנייה)");
  assert.match(lastBody(), /23/, "עזיבה 23/7 (19/7 + 4 לילות מההודעה הראשונה)");
  assert.match(lastBody(), /4 לילות/, "ארבעה לילות (מההודעה הראשונה — לא נשכח)");
});

test("צבירת תאריכים: הסדר ההפוך — '19.7' ואז '4 לילות' — גם עובד", async () => {
  const p = await checkinUpTo("waiting_dates");
  ...
  await bot.handleIncoming(p, "4 לילות");
  assert.match(lastBody(), /נכון\?/, "אחרי שני החלקים — מבקש אישור");
  assert.match(lastBody(), /19/, "הגעה 19/7");
  assert.match(lastBody(), /23/, "עזיבה 23/7");
});
