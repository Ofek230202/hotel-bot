...
// ════════════════════════════════════════════════════════
//  בינלאומי — דרכונים ות"ז מכל העולם (חברה בינלאומית)
// ════════════════════════════════════════════════════════
test("בינלאומי: דרכונים ות\"ז מכל מדינה מתקבלים; הלאום נשמר כשדה", async () => {
  const { MockIdProvider } = await import("./idverify/MockIdProvider.js");
  const provider = new MockIdProvider();
  const written = [];
  const prev = process.env.ID_STORE_MODE;
  delete process.env.ID_STORE_MODE; // verify-then-discard → אין קבצים לנקות

  // מגוון מסמכים אמיתיים מרחבי העולם — כולם ת"ז/דרכון, לאום שונה.
  const cases = [
    { docType: "passport", nationality: "United States" },
    { docType: "passport", nationality: "Japan" },
    { docType: "id_card",  nationality: "Germany" },
    { docType: "id_card",  nationality: "Spain" },
    { docType: "passport", nationality: "India" },
    { docType: "id_card",  nationality: "Israel" },
    { docType: "passport", nationality: "Brazil" },
  ];
  try {
    for (const c of cases) {
      visionResult = { valid: true, isId: true, readable: true, confidence: 0.95,
        docType: c.docType, fields: { full_name: "Test Guest", document_type: c.docType, nationality: c.nationality },
        reasonHe: "", reasonEn: "" };
      const r = await provider.verifyDocument({ mediaUrl: "https://x/1", contentType: "image/jpeg" });
      if (r.storedPath) written.push(r.storedPath);
      assert.equal(r.status, "verified", `${c.nationality} ${c.docType} חייב להתקבל`);
      assert.equal(r.fields?.nationality, c.nationality, "הלאום חולץ ונשמר");
    }
  } finally {
    if (prev !== undefined) process.env.ID_STORE_MODE = prev;
    for (const f of written) for (const p of [f, f.replace(/\.\w+$/, ".json")]) { try { fs.unlinkSync(p); } catch { /* ignore */ } }
  }
});

test("בינלאומי: סלפי / צילום מסך / תמונה אקראית עדיין נדחים", async () => {
  const { MockIdProvider } = await import("./idverify/MockIdProvider.js");
  const provider = new MockIdProvider();
  for (const bad of ["selfie", "other"]) {
    // ה-vision מסמן valid=false על מה שאינו מסמך (סלפי/צילום מסך/אקראי).
    visionResult = { valid: false, isId: false, readable: false, confidence: 0.2,
      docType: bad, reasonHe: "נא לשלוח תעודת זהות או דרכון.", reasonEn: "Please send an ID card or passport." };
    const r = await provider.verifyDocument({ mediaUrl: "https://x/1", contentType: "image/jpeg" });
    assert.equal(r.status, "rejected", `${bad} חייב להידחות`);
    assert.equal(r.storedPath, null, "לא נשמר");
  }
});

// (בדיקת תוכן ה-prompt הבינלאומי נמצאת ב-idsecurity.test.mjs — כאן
//  vision.js ממוקה, ולכן SYSTEM אינו זמין.)
...
test("חירום (רפואי, עברית): האורח מקבל מיד 101 + כל המספרים, בלי הנחיה רפואית, והביטחון מוסלם", async () => {
  ...
  assert.match(guest, /אין לי הסמכה|לא מוסמך|איני מוסמך/, "הבהרה (ניטרלית-מגדר) שאינו נותן הנחיות רפואיות");
  assert.ok(!guest.includes("["), `תג דלף לאורח: ${guest}`);
